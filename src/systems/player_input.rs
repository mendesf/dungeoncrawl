    ...
    if let Some(key) = key {
        let delta = match key {
            ...
            VirtualKeyCode::G => {
                ...
                items.iter(ecs)
                    .filter(|(_entity, _item, &item_pos)| is_next_to(player_pos, item_pos))
...
fn is_next_to(player: Point, item: Point) -> bool {
    (player.x == item.x && (player.y - 1 == item.y || player.y + 1 == item.y))
        || (player.y == item.y && (player.x - 1 == item.x || player.x + 1 == item.x))
}
